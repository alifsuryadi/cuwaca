...
app.post("/cuwaca-akses", async (req, res) => {
    ...
    try {
        const response = await axios.get(`${API_URL}data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${API_Key}`);
...
app.post("/cuwaca-city", async (req, res) => {
    ...
    try {
        const response = await axios.get(`${API_URL}data/2.5/weather?q=${city},${country}&APPID=${API_Key}`);
        const result = response.data;
